...
export default async function Parking() {
  ...
  try {
    user_community_id= await sql`SELECT community_id FROM user_info WHERE email = ${session?.user?.email}`
  ...
  try {
    parking_DB = await sql`SELECT parking_space.id, parking_space.number, visitor_vehicle.brand, visitor_vehicle.model, visitor.firstname, visitor.lastname, user_info.firstname as resident_firstname, user_info.lastname as resident_lastname
                             FROM parking_space 
  ...
  try {
    ocupied_spaces= await sql`SELECT * FROM parking_space_usage INNER JOIN parking_space ON parking_space_usage.parking_space_id = parking_space.id WHERE community_id = ${user_community_id}`
  ...
  logger.info(`User with community_id '${user_community_id}' has opened the parking page.`);
  ...
  return (
    ...
    <Parking_Component db_parking_space = {db_parking_space} db_parking_space_usage = {db_parking_space_usage} ocupied_spaces= {ocupied_spaces} parking_DB = {parking_DB}/>
    </div>
  );
